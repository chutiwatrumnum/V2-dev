...
const CreateBuildingActivities = (props: ComponentCreateProps) => {
  ...
  const onFinish = async (values: any) => {
    confirm({
      ...
      content: "Are you sure you want to create building activities?",
      icon: null,
      okText: "Yes",
      okType: "danger",
      cancelText: "Cancel",
      centered: true,
      async onOk() {
        const request: BuildingActivitiesNew = {
          typeId: values?.event,
          title: values?.title,
        ...
        const reultCreated = await addBuildingActivity(request);
        if (reultCreated) {
  ...
  return (
    <>
      <Modal
        title="Create building activities"
        width={800}
        centered
        open={props?.isOpen}
...
export default CreateBuildingActivities;
