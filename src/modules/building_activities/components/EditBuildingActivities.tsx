...
interface EditBuildingActivitiesProps {
  BuildingActivities: any;
  isOpen: boolean;
  callBack: (isOpen: boolean, saved: boolean) => void;
}
let emailGroup: any[] = [];
const EditBuildingActivities = (props: EditBuildingActivitiesProps) => {
  const { token } = theme.useToken();
  const [tags, setTags] = useState<string[]>([]);
  const [inputVisible, setInputVisible] = useState(false);
  const [inputValue, setInputValue] = useState("");
  const [defaultEmailGroup, setDefaultEmailGroup] = useState<string[]>([]);
  const [eventMasterDataList, setEventMasterDataList] = useState<
    eventMasterGroupList[]
  >([]);
  ...
  const onFinish = async (values: any) => {
    const request: BuildingActivitiesEdit = {
      id: props?.BuildingActivities.key,
      typeId: values?.event,
      title: values?.title,
    ...
    await Modal.confirm({
      ...
      content: "Are you sure you want to edit maintenance facilities?",
      icon: null,
      okText: "Yes",
      okType: "primary",
      cancelText: "Cancel",
      centered: true,
      async onOk() {
        const reultEdited = await editDataBuildActivity(request);
        if (reultEdited) {
  ...
  const initedit = async () => {
    const editdata: any = props?.BuildingActivities;
    ...
    const resultDataEventMaster: any = await getdataEventMasterlist();
    if (resultDataEventMaster?.status) {
      resultDataEventMaster?.datavalue.map((e: any) => {
        if (e.label === editdata?.calendarTypeName) {
          editdata.typeId = e.value;
        }
      });
      await setEventMasterDataList(resultDataEventMaster?.datavalue);
    }

    await setemailGroupDefaultSelected(editdata?.sendMailGroupId);
    const dateInitEdit = dayjs(editdata?.date, "YYYY-MM-DD");
    await form.setFieldsValue({
      event: editdata.typeId,
      ...
      note: editdata?.note ? editdata?.note : null,
    });
    ...
    if (e > -1) {
      if (emailGroupDefaultSelected !== e) {
        ...
        const diffEmail = tags.filter((e: any) => {
          if (!props?.BuildingActivities?.sendMailGroup.includes(e)) {
            return e;
          }
        });
        await setDefaultEmailGroup([]);
        await setTags([
          ...props?.BuildingActivities?.sendMailGroup,
          ...diffEmail,
        ]);
        await form.setFieldsValue({
          emailGroup: props?.BuildingActivities?.sendMailGroup,
        });
      }
    } else {
  ...
  return (
    <>
      <Modal
        title="Edit building activities"
        width={800}
        centered
        open={props?.isOpen}
...
export default EditBuildingActivities;
