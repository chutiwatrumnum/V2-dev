  ...
  effects: (dispatch) => ({
    async getTableData(payload: EmergencyPayloadType) {
      try {
        ...
        const result = await axios.get("/emergency-call/dashboard/list", {
          params,
        });
    ...
    async deleteTableData(payload: number) {
      try {
        const result = await axios.delete(`/emergency-call/${payload}`);
    ...
    async addNewEmergencyService(payload: DataEmergencyCreateByType) {
      try {
        const result = await axios.post("/emergency-call", payload);
    ...
    async editEmergencyService(payload: any) {
      try {
        const result = await axios.put("/emergency-call", payload);
        if (result.status >= 400) {
          console.error(result.data.message);
